...
const MOCKS = {
  ...
  '@supabase/supabase-js': 'export const createClient = (url, key) => ({ url, key, auth: { getSession: async () => ({ data: { session: { access_token: "test-token" } }, error: null }) }, from: () => ({ select: () => Promise.resolve({ data: [], error: null }) }) });',
  '@simplewebauthn/browser': 'export const startRegistration = async (opts) => { if (globalThis.__swaBrowserMock?.startRegistration) return globalThis.__swaBrowserMock.startRegistration(opts); throw new Error("startRegistration not mocked"); }; export const startAuthentication = async (opts) => { if (globalThis.__swaBrowserMock?.startAuthentication) return globalThis.__swaBrowserMock.startAuthentication(opts); throw new Error("startAuthentication not mocked"); };',
};
