-- ⚠️  DEPRECATED / HISTORICAL SCHEMA — DO NOT RUN
-- -------------------------------------------------------
-- This file used BIGINT GENERATED BY DEFAULT AS IDENTITY
-- primary keys.  The live schema was migrated to UUID PKs
-- via migration_v2.sql (schema_realtime.sql is the current
-- canonical reference).
--
-- Applying this file against the live database will create
-- an incompatible schema.  It is kept for historical context
-- only.  Use supabase/schema_realtime.sql as the base and
-- supabase/security_hardening.sql for RLS policies.
-- -------------------------------------------------------

-- UNIZIK ATTENDANCE PWA SCHEMA (ORIGINAL — BIGINT PKs)
