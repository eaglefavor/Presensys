        ...
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 1050 }}>
          <motion.div className="modal-dialog modal-fullscreen-sm-down modal-xl modal-dialog-centered px-3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            <div className="modal-content border-0 shadow-2xl rounded-4">
              ...
              <div className="modal-body p-0 bg-light" style={{ minHeight: '400px' }}>
                {showMapper && uploadedFile ? (
                  ...
                  <div className="p-4 d-flex flex-column gap-2">
                    <button className="btn btn-white p-3 rounded-3 border-0 shadow-sm text-start d-flex align-items-center gap-3 bg-white" onClick={() => setImportMode('manual')}><div className="bg-primary bg-opacity-10 text-primary p-2 rounded-2"><Plus size={24} /></div><div><div className="fw-bold small">Manual Entry</div><div className="xx-small text-muted">Individual student addition</div></div></button>
                    <button className="btn btn-white p-3 rounded-3 border-0 shadow-sm text-start d-flex align-items-center gap-3 bg-white" onClick={() => setImportMode('paste')}><div className="bg-warning bg-opacity-10 text-warning p-2 rounded-2"><ClipboardPaste size={24} /></div><div><div className="fw-bold small">Smart Paste</div><div className="xx-small text-muted">Import from text lists</div></div></button>
                    <button className="btn btn-white p-3 rounded-3 border-0 shadow-sm text-start d-flex align-items-center gap-3 bg-white" onClick={() => setImportMode('file')}><div className="bg-success bg-opacity-10 text-success p-2 rounded-2"><FileText size={24} /></div><div><div className="fw-bold small">Excel / CSV</div><div className="xx-small text-muted">Bulk spreadsheet upload</div></div></button>
                  </div>
                ) : importMode === 'manual' ? (
                  <div className="p-4">
                  ...
                  <div className="p-4 h-100">
                    {parsedStudents.length === 0 ? (
                      <div className="d-flex flex-column gap-3 h-100"><textarea className="form-control border-0 bg-white p-3 rounded-3 shadow-inner font-monospace flex-grow-1" style={{ minHeight: '300px', fontSize: '13px' }} placeholder="Paste names and numbers here..." value={pasteData} onChange={e => setPasteData(e.target.value)} /><button className="btn btn-primary w-100 py-3 rounded-3 fw-bold shadow" onClick={handleParse}>PROCESS TEXT</button></div>
                    ) : (
                      <div className="d-flex flex-column gap-3">
                        <div className="d-flex justify-content-between align-items-center"><h6 className="fw-black text-success small mb-0">{parsedStudents.length} STUDENTS READY</h6><button className="btn btn-link text-danger xx-small fw-bold text-decoration-none" onClick={() => { setParsedStudents([]); setPasteData(''); }}>RESET</button></div>
